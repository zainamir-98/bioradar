...
def main():
    ...
    interrupt_handler = utils.ExampleInterruptHandler()
    print("Calibrated range interval: %.2f cm to %.2f cm\n" % (START_DISTANCE, STOP_DISTANCE))
    ...
    while not interrupt_handler.got_signal:
        ...
        breathing, hr, hrv, dist, rbm = processor.process(data)
        ...
        if (x <= 0):
            ...
            print("Acquiring HRV and HR.\nHold your breath for %s seconds.\n" % str(WINDOW_LENGTH))
            print(load_bar_str)
            load_bar_str = "["
            ...
            if hr is not 0:
                print("> Heart rate variability: %.2f ms" % hrv)
                print("> Heart rate: %.2f beats/min" % hr)

                data = {"BR": 0, "HR": hr, 'HRV': hrv}
...
def get_sensor_config_envelope():
    ...
    config.update_rate = RADAR_FS_ENV
    config.gain = 0.5
...
class VitalsProcessor:
    def __init__(self, sensor_config, processing_config, session_info):
        ...
        self.r_b = firwin(300, 0.5, window=('kaiser', 1), fs=self.radar_freq)
    ...
    def process(self, sweep):
        ...
        if self.buffer_filled:
            ...
            self.hrv = self.find_hrv()
           # except:
                #print("Encountered an error in HR/HRV estimation.\nClosing window in 10 seconds.")
                #sleep(10)
        ...
        self.r_buffer, _ = sosfilt(self.r_sos, self.signal_buffer, zi=self.r_zi)
        self.detect_breathing()
        ...
        return self.breathing, self.hr, self.hrv, self.dist, self.rbm_detection()
...
#                 d = self.exp_ma(self.dist_array[p], self.dist, 0.99)
    ...
    def find_hrv(self):
        peak_idx, peak_mag = find_peaks(self.h_buffer, prominence=0.1, height=0)
        
        peak_mag = peak_mag["peak_heights"]
        print(np.shape(peak_idx))
        print(np.shape(peak_mag))
        
#         med_mag = np.median(peak_mag)
#         
#         for p in range(len(peak_mag)):
#             if peak_mag[p] > med_mag*1.8:
#                 peak_idx = np.delete(peak_idx, p)
#                 peak_mag = np.delete(peak_mag, p)
#                 print("Removed!")
        
        ibi = np.array([])
        hrv = np.array([])
        
        for p in range(1, len(peak_idx)):
            ibi = np.append(ibi, (peak_idx[p]-peak_idx[p-1])/self.radar_freq)
        
        ibi *= 1000
        
        for i in range(1, len(ibi)):
            hrv = np.append(hrv, (ibi[i]-ibi[i-1])**2)
            
        axi = np.linspace(0, WINDOW_LENGTH, len(self.h_buffer))
        
        hrv = np.sqrt(np.mean(hrv))
        
        if DEBUG_PLOT:
            plt.plot(axi, self.signal_buffer)
            plt.plot(axi, self.h_buffer)
            plt.plot(axi[peak_idx], self.h_buffer[peak_idx], "x")
            plt.show()
        return round(hrv, 2)
    ...
    def detect_breathing(self):
        ...
        if inhale_time is not None and exhale_time is not None:
            self.rr = 60.0 / ((inhale_time + exhale_time) / self.radar_freq)
